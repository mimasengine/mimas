...
# Boot header: project-identity IP.BIN generated by the pre_build hook
# (pre.makefile -> tools/make_ip.py): maker N0rt0N85, product Mimas-Eng,
# title "Mimas Engine", version V0.<git commit count>, date = build date.
# Consumed by shared.mk's
# xorrisofs -generic-boot; needs the SRL_IPBIN stray-';' hunk of
# patches/saturnringlib.patch to expand cleanly.
SRL_IPBIN  = $(BUILD_DROP)/IP.BIN
...
# Flicker-test build (core/g_game.c G_DoLoadLevel): SAT_TEST_GOD=1 spawns the player INVINCIBLE +
# fully kitted (god mode, all weapons/keys/ammo, armor) so a HW tester can stand in a monster horde
# and film the VDP1 V1 probe without dying / being blocked.  Empty (default) = normal.  Pair with
# SAT_WARP_MAP to pick the arena, e.g. make SAT_WARP_MAP="1 7" SAT_WARP_SKILL=4 SAT_TEST_GOD=1.
SAT_TEST_GOD ?=
ifneq ($(SAT_TEST_GOD),)
