...
| 17 | `dg_saturn.cxx` (split only) | `SPL <v0> <v1> <v2> <v3> k<kick> =<sum> tb<tc><bal> h<ms> p<c><n>` — per-view render ms, the VDP1 kick, their sum, then the two live split knobs and the split-HUD paint. 🔴 **`tc<n> bal<n>` became `tb<tc><bal>` on 2026-08-25** because the row **overran 40 cells in 4p and lost `po`'s value off the right edge** — visible in the 08-25 captures as a bare `po` with no digits. Same two knobs (pad L+Left = thing-cull, pad L+Right = SQ-balance rotation), five cells cheaper. ⚠ In **3p** the 4th slot is `sat_spl_v3 + sat_spl_mmap`, i.e. a view **plus** a cost centre, in that mode only. | player-count |
