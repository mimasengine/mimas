...
|-----|-------|---------|--------|
| 0 | `dg_saturn.cxx:1983` | fps / MST / `to<rate>:<site>` / `cd<retries>` / `ld<chunks>/<refaults>` — 🔴 **2026-08-28: `ld` GAINED THE RE-FAULT WITNESS, `to` LOST ITS FOUR DIGITS, AND THE ROW NOW PADS AND CUTS.** It was rendering at **exactly 40 content cells** in a calm 4p frame (`5.5fps a9.3 MST181 to0:0000 cd0 ld2075/4`), so the old denominator — `sat_cd_persector`, the settled R1 baseline — **was already being sliced**: that `4` is the first digit of a longer number. Same defect class as the row-14 `lk` truncation found two days earlier, and just as invisible. **`<refaults>` = `w_lump_refaults` (core `w_wad.c`): lumps pulled off the medium whose lump had already been pulled once this run.** It exists because `ld` climbed **583 → 2075** across one owner session while [[streaming-load-budget-and-flat-treadmill]] says it **must plateau**, and `CD t` moved **61 s → 64 s over 103 loads** between two 1p captures — **29 ms per load, three seconds of disc between two screenshots**. Two worlds, opposite cures, and nothing on screen could separate them: **refaults FLAT while `ld` climbs = CHURN** (every load is a lump never seen — no cache helps, the cure is R2.3 async) versus **refaults CLIMBING = THRASH** (the zone evicts lumps under the renderer and they are pulled twice — the cure is R4 residency, and async would only hide it). ⚠ **READ IT AS «does it move», NOT as a ratio**: the numerator counts GFS *chunk commands* (several per big lump, platform-side), the denominator counts *lump* re-faults (core). Different granularities **on purpose** — the numerator keeps its meaning so every capture already taken stays comparable. ⚠ Cart builds read 0/0 by construction (the memcpy path is excluded). The bitmap is one bit per lump, `Z_Malloc` PU_STATIC once (~390 B for TNT), from the **zone**, not the TLSF pool. **`to`'s four per-site digits became one dominant-site letter** (`A`/`P`/`M`/`W`, `-` when the rate is zero): they read `0000` in every capture ever taken and only carry information when the rate is non-zero, where the letter still says which site. 🔴 **`a` (the EMA) WAS BROKEN until 2026-08-26 and the break only ever hid GAINS.** It was `avg10 = (avg10*3 + inst10)/4` in tenths of an fps: every `a` in `(inst-4, inst]` is a fixed point of the truncating divide, so a RISE stalled 0.3 fps short **for ever** while a FALL converged exactly. Owner caught it on the wall-fill A/B — `inst` pinned at 8.4, `a` pinned at 8.1, indefinitely. The EMA is now carried at ×4 and rounds. ⚠ **Any A/B photographed before 2026-08-26 read its improvement up to 0.3 fps low**, which at 7-8 fps is ~4 %. ⚠ alpha is UNCHANGED at 1/4 per **second**: “~4 s” is the time constant (63 %), NOT the settling time — an 0.8 fps step still needs **~9 s** to close. Read `inst` for a step, `a` only once it has stopped moving. ⚠ **`MST` is `1000/inst`, so it is FIELD-QUANTIZED, not a work measurement**: NTSC fields are 16.683 ms, so 7 fields = 116.8 ms and 8 = 133.5 ms and nothing lands between them. Work is `R+T+S+b+dg` on row 1; `MST` minus that sum is the **vblank wait**. The 2026-08-26 4p TNT A/B is the worked example: work fell **4 ms** (115.8→111.8) and `MST` fell **12.6** (131.6→119.0) because 115.8 sat 1 ms under the 7-field ceiling and 111.8 sits 5 ms under it — the extra 8.6 ms was idle wait being reclaimed, not work removed. Read a `MST` jump against row 1 before calling it a win of that size. | — |
...
| 9–10 | `dg_saturn.cxx:2378/2381` | `FMp <p50>/<p90>/<p99> sk<KB> mx<n> hp<peak>/<size>!<fail>` — 🔴 **THESE ARE FRAME-MILLISECOND PERCENTILES, NOT FREE MEMORY** (the name misleads; `mh_ms` is a frame-time histogram). 🔴 **`MH_MS_SHIFT` 3 → 4 ON 2026-08-28 BECAUSE p99 WAS PINNED AT THE TOP BUCKET IN EVERY CAPTURE EVER TAKEN.** 40 buckets × 8 ms capped the histogram at **320 ms**, so `p99` read **312 in all 21 frames** of the owner's stutter session — not because the tail sits at 312 but because the instrument could not look further, while the same session photographed **MST 588, 526, 500, 322** by hand. `mx` could not stand in: it is a *session* max pinned at the level-load spike (`mx6586`), so everything between 312 ms and 6.6 s was invisible — **in the one field that measures the complaint**. Now 16 ms buckets → **0–640 ms**, which covers the worst frame actually observed with margin. The resolution given up is 16 ms on a p50 of ~50, i.e. **one NTSC field** — the unit `MST` is quantised in anyway. ⚠ Any `FMp` read before 2026-08-28 has a **p99 that means «≥312», not «312»**. 🔴 **AND WIDENING THE BUCKETS DID NOT UNPIN IT — THE BUG WAS THE WINDOW, FIXED LATER THE SAME DAY.** `mh_reset` fires only on a mode/SQ change, i.e. essentially never, so **both** halves of this row described the whole run since boot: `mx` was pinned at the level-load spike (`mx6601` in all 23 captures of the 08-28 TNT Ymir pass, `mx11934` on console) and the percentiles carried every frame ever rendered, load frames included. With a 6.6 s frame and hundreds of slow ones permanently in the tail, **any** top bucket is reached by 1 % of a run-long sample: p99 read **624 in 22 of 23** captures exactly as it had read 312 before. Now **two cadences, because the two statistics want different ones**: `mx` rolls on ~1 s of accumulated frame time (self-timed from the samples, no external hook) so it answers *«did the last second stutter»*, which is the actual complaint — a single 6.6 s load frame is reported once and then gone; the histograms **halve** every 256 samples instead of clearing, giving an exponential ~256-frame window with no cold start and no percentile hole after a fold. ⚠ `mh_pct` now sums the array it is handed instead of trusting `mh_frames`: halving rounds each bucket down independently, so the three arrays stop sharing a total, and a denominator larger than the real sample count makes the loop run off the end and return the **last bucket** — the exact false saturation this change exists to remove. ⚠ **`mx` is no longer comparable with any capture before 2026-08-28**: it was a session max, it is now a ~1 s max. | — |
| 11 | `dg_saturn.cxx` | `LIM vp<peak>.<poolovf-PLANES> ds ss[!] o<rows> zf<n> lg<n> ca<n>` — **exactly 40 cells** (pad, then `ovbuf[40]` cut). 🔴 **`ca` ADDED 2026-08-28 = THE RESIDENT LUMP CACHE IN KB** (`Z_FreeMemory` − `Z_TrueFree`, i.e. free+purgeable minus free). It ships **with** the recency purge because it is the number that says whether that purge *can* work: an approximate LRU beats address order only when the cache is comparable to the working set, and when it is much smaller **every** eviction policy performs identically. Nothing on screen could say which world we are in — and the same morning's `zf` → true-free change made it worse by removing the only witness to free+purgeable. **Read the three together**: `zf` = room to allocate **without evicting anything**, `ca` = how much cache is standing, `lg` = the largest contiguous run purging could produce. A **big `ca`** with re-faults still climbing on row 0 ⇒ the **policy** is wrong (recency should bite); a **small `ca`** ⇒ there is no cache to manage and the lever is **capacity, not policy** — and the zone census in `z_zone.c`'s header names where the capacity went (`st438K + lv377K` on TNT MAP11: map geometry 249 K, `lumpinfo` 72 K, the visplane span pool 60 K, the lead-fill ring 36 K, the DRP table 26 K). 🔴 **`zf` CHANGED MEANING ON 2026-08-28: IT IS NOW TRULY FREE (`PU_FREE` only), NOT free+purgeable.** It read `Z_FreeMemory`, which counts every `PU_PURGELEVEL` block — i.e. **the whole lump cache** — as available. So `zf` and `lg` were two views of the *same* assumption (that the cache is expendable) and **neither could say how much of the zone is actually unused**. That stopped being a cosmetic redundancy the day the re-fault witness proved the cache *is* being evicted: `zf441` was read here as «the zone has room, so this is not starvation» and it was nothing of the kind — it was free **plus** cache. Now the pair reads **truly free vs obtainable-by-purging**, and **the gap between them is the resident cache**. That gap is also the receipt for `Z_Malloc`'s non-purging pass (2026-08-28): if the pass fires, the cache grows and **`zf` falls**. If refaults on row 0 fail to drop, `zf` says which failure it is — still large ⇒ the pass is not firing; near zero ⇒ the zone really is full and there was nothing to keep. Same width, same 999 clamp. ⚠ the old legend was stale twice over: `op tc rl` moved to **row 22** on 2026-08-10, and the cited line number was ~800 lines out. **`o` ADDED 2026-08-25, and REDEFINED the same day: it is the high-water of DEMAND, not of CONSUMPTION.** 🔴 The consumption form **saturated by construction** — all three sinks (`r_segs.c` masked-column, `sprtopclip`, `sprbottomclip`) redirect into `opening_overflow` **without advancing `lastopening`**, so the instant the array is too small the counter pins at `MAXOPENINGS` and can never say *by how much*. Cutting the array on a consumption reading would therefore have destroyed the one instrument able to size the cut. `r_opening_demand` now accumulates what every caller **asked for**, sink or not, so the reading stays meaningful at any array size and the cut is order-independent forever. It is the ~1 s high-water in 320-word rows, against the **16** in `#define MAXOPENINGS SCREENWIDTH*16` (`r_plane.c`) — **cut from 64 on 2026-08-26, the first time that number was ever set by measurement**; vanilla's 64 was a guess whose entire comment was `// ?`, and at 40 960 B of .bss it was larger than the whole TLSF pool it was starving. Row-22 `op` is the overflow-redirect COUNT, the alarm that only rings once it is already too late; `o` is the number that **sizes** the array. Paid for without losing a field: the two literal `k` suffixes went (the legend carries the unit), `zf`/`lg` are **clamped at 999** (the zone is 1016 KB, so a 4-digit reading at level load is exactly what used to push this row to 41 cells and drop the last one silently), and the pool-overflow digit is now printed as **PLANES** (already halved, clamped 99) — **do not halve it again**. ⚠ `o` is a high-water, NOT a ceiling: the theoretical worst is 3 writes × width × `MAXDRAWSEGS` = 245 760 words, 12× the array, so `MAXOPENINGS` stays a probability bet backstopped by the garde. **FIRST READINGS, 2026-08-25 (Ymir, a COUNT so the emulator is authoritative): `o1` or `o2` on 13/13 captures — shareware E1M1 AND TNT MAP01, 1p through 4p, action included, with row-22 `op0` throughout.** `o1` means ≤320 of the 20 480 words were consumed of the then-20 480, i.e. ≤1.6 %. **CUT 2026-08-26 to 16 rows** (5 120 words, 10 240 B). Measured effect on the linker map: `__heap_start` 0x060f2e60 → 0x060eb800, so the **TLSF pool goes 29 088 → 59 392 B — it more than doubles** (+30 304; the 416 B short of the arithmetic is section alignment). 16 rows and not the 8 the arithmetic allowed: `o` is a high-water, **not a ceiling** — the theoretical worst is 3 writes × width × `MAXDRAWSEGS` = 245 760 words, 12× even the OLD array — so the extra 5 KB of margin is cheap beside a 30 KB win, and the **garde remains the real safety net, not the size**. ⚠ The pre-cut check the previous revision of this legend demanded IS DONE: row-4 `tl` reads **0.7 ms** on the same TNT 4p frame, so the four `openings` memcpy are running and `o1` is genuine headroom — not a Mimas wall mode silently skipping the silhouette store. ⚠ Watch row-22 `op`: non-zero on a real map means RAISE `MAXOPENINGS`, never remove the garde. ⚠ The `vp` field two columns to its left **was overflowing on the same frames** (`vp66.2`/`vp67.8` on TNT 4p against `VP_POOL_PLANES=64`) — a 40 KB array sitting idle beside a pool that was running out. **Closed 2026-08-26**: sizing the three visplane slice arrays to `viewwidth` instead of `SCREENWIDTH` reads **`vp67.0`** on the same scene — same 67-plane peak, zero redirects, `VP_POOL_PLANES` untouched. | — |
| 12 | `dg_saturn.cxx` | **TWO TENANTS, mutually exclusive by ONE predicate** 🔴 **WHICH CHANGED ON 2026-08-28: it is now `sat_wad_base == nullptr && sat_local_players <= 1`, so SPLIT ALWAYS GETS `SKY`, disc or cart.** The old build-only predicate made `SKY` — the row whose own definition says it exists to price the multi-view HW-sky plan — **invisible on the only disc the console session ever runs**, a `-Repack` CD build. A probe written to size a plan, on a row that plan's own test can never reach, is not a probe. Player count is the honest split: `SKY`'s four slots are forced to 0 below two players, so it carries nothing in 1p, while the `CD` gardes are a 1p-streaming story. 🔴 **THAT COST WAS PAID BACK THE SAME DAY — `!<gardes>` NOW RIDES THE `SKY` HALF.** The stated price was «in co-op we lose `t`/`px`/`ob`/`gy`/`st`», and it was too high to leave standing: `px` is a **converted crash**, `ob` is the owner's «wrong texture for one frame», `st` is a wrong texture drawn by the *slave* — three counters that **must read zero**, taken off screen in the mode with the most zone pressure and the mode the console session actually runs in. **A capture with a non-zero garde is not a measurement**, so the field that says so has to exist in every mode a capture is taken in. `!` is their SUM (they share one meaning: *this photo is suspect*) and it sits **immediately after `e`**, because the pad-and-cut must never be what hides it — both truncations found this week (`lk`, `ld`) happened at a row's tail. `gy` and `t` stay 1p-only: `gy` is **expected** non-zero (mixing it into `!` would destroy the must-be-zero property) and is visible in split from two other sides anyway — V1 `fl<slot>` and VRM `lb..<nocol>`; `t` is secondary now that row 1 carries `ld<chunks>/<refaults>` in every mode. **CD-streaming, 1p**: `CD pn<total>/<max> t<s> px ob gy st` 🔴 **`ip` ANSWERED TWICE AND WAS REPLACED BY `pn` (2026-08-29).** First it named the **DRP LZSS scratch** — `@5dc04` = `sat_drp_read_lump_n`, **454 of E1M1's 659** in-play long-lived allocations and **523 of E1M2's 759**, a `Z_Malloc(PU_STATIC)` per *compressed lump*; one persistent buffer removed it and the site vanished from every later capture (`ip` fell 659 → 147 in count, **831 KB → 36 KB** in bytes). After that it reported nothing but `R_GenerateLookup`'s two column directories, which that function retags `PU_CACHE` before returning — exactly the over-count documented here. **A field whose only remaining output is its own documented artefact does not keep twelve cells.** `pn<total>/<max>` = the pinned bytes (every `tag < PU_PURGELEVEL`) and the biggest single one, in KB, from `Z_LargestAllocatable`'s own pass. 🔴 **WHY THE SUBJECT CHANGED**: re-anchoring the rover at each level load (`Z_ResetRover`) moved row-11 `lg` **336 → 491** on E1M1 and **128 → 279** on E1M2 — +46 % and +118 % of contiguity — **and the re-fault rate did not move** (27 % / 50 % against 26 % / 45 %). Contiguity has stopped being what binds. What binds is one row up: `zf` reads **3–33 KB** across the whole of E1M2 while `ca` holds ~400. The zone is **saturated** — cache plus a rounding error of free space — so every new texture evicts an old one however tidily the space is arranged, and the only lever left is **to pin less**. Nothing on screen had ever said how much *is* pinned. **Read `pn` + `ca` + `zf` together**: they account for the whole 1016 KB zone, and `max` says whether one block dominates. ⚠ `t<s>` comes back with the three cells the return address gave up. Worst case is 37 of 40 cells 🔴 **REVISED THE SAME DAY (2026-08-29): THE LATCH IS THE MOST *FREQUENT* SITE, NOT THE BIGGEST, AND `t<s>` IS RETIRED TO PAY FOR IT.** The first version latched the `ra` of the LARGEST in-play allocation and it named a **one-shot**: `@32d98` on every capture of the owner's session, which resolves inside `R_WallFillArm` — the 36 KB lead-fill span ring, claimed once on the first rendered frame. A real find (it is now claimed at `R_Init` instead, see `R_LeadFillInit`), but a biggest-slot cannot answer *who keeps allocating*, which was the whole question. Now a 16-slot open-addressed table counts hits per return address and the row prints **the top site's count over the total**, so dominance is readable at a glance: `ip412/692` = one caller did 60 % of them; `ip8/692` = it is spread and the fix has to be structural. ⚠ **THE TOTAL OVER-COUNTS ON PURPOSE AND YOU MUST KNOW IT**: the tag is read *at allocation*, and several callers allocate `PU_STATIC` then immediately retag `PU_CACHE` (`R_GenerateComposite`'s composite, `R_GenerateLookup`'s two column directories) or `Z_Free` before the frame ends (`patchcount`). Those are counted and are **not** permanent fragmenters. That is why the field reports a SITE and not just a number — resolve the `ra` against `build/Mimas-<Wad>.map`, read that function, decide. A probe that silently guessed which allocations were transient would be worse than one that says so. ⚠ `t<s>` (cumulative CD time) is gone: row 0 `ld<chunks>/<refaults>` carries the disc story in every mode, and this legend had already called `t` secondary once `ld` gained the re-fault witness. ⚠ `lg` on row 11 stays the **verdict**; `ip` is only the diagnosis. ⚠ px/ob/gy are clamped to ONE digit: any value above zero already voids the photo. Worst case is exactly 40 cells 🔴 **`sp` LIVED ONE DAY (2026-08-29) AND WAS REPLACED BY `ip`, HAVING ANSWERED.** Over the owner's 54 shareware 1p captures `sp` said, in order: early in a level the best single relocation is worth **+208 KB** (`sp442L2` against `lg234` — a 2 KB `PU_LEVEL` block splitting 234 | 206); then `lg` falls **424 → 267 in ONE step** (two lumps loaded, `ld` +2 chunks / +2 re-faults); and from that moment the best block is a **4 KB `PU_STATIC` worth only `lg`+4**. **`gain − lg` equal to the block's own size means the free run AFTER it is zero** — it is flanked by *another* unpurgeable block. **They come in clusters**, so no single relocation recovers the run, and « relocate the splitter » died as a plan the moment it was measured. That was the stated failure mode, checked for on purpose, and it fired. The question is no longer *which* block but **who keeps allocating them** — which is `ip`: long-lived allocations (`tag < PU_PURGELEVEL`) made **since `P_SetupLevel` finished**, their total in KB, and the **low 20 bits of the return address of the biggest**, resolved against `build/Mimas-<Wad>.map` for a function name. ⚠ 20 bits, not 32: the ELF loads at `0x060xxxxx` and is a few hundred KB, so five hex digits are unique and the row keeps five cells. ⚠ **One latched `ra`, not the per-block `SAT_ZONE_RA` field** — that one costs 4 bytes on *every* header and would shift every allocation in the zone, i.e. perturb the exact layout being measured. ⚠ `lg` on row 11 stays the **verdict**; `ip` is only the diagnosis. ⚠ px/ob/gy are now clamped to ONE digit: any value above zero already voids the photo, so the count above 9 changes nothing, and the cells pay for `ip`. Worst case is 39 of 40 cells 🔴 **`sp` ADDED 2026-08-29 = THE ZONE SPLITTER, AND IT IS PLACED FIRST, RIGHT AFTER «CD », SO THE CUT CAN NEVER REACH IT** (the row now pads and cuts at 40; worst case with every clamp at its ceiling is exactly 40, so nothing is cut in practice and the cut is a belt). Relocating **one** unpurgeable block of `<size>` KB, of class `<T>`, would take the largest contiguous run to `<gain>` KB — so read `gain` against **`lg` on row 11: it is what `lg` WOULD be**. `T`: `S` static, `O` sound, `M` music, `L` level, `V` levspec, `?` other (literals 1/2/3/5/6 — ⚠ **not** vanilla Doom's 50/51; this enum has no explicit values past `PU_STATIC`, and the `Z_Malloc` halt message has said 50/51 since it was written and is wrong about it). Match `<size>` against the zone census in `z_zone.c`'s header — LineDefs 101 K, `lumpinfo` 72 K, the visplane span pool 60 K, the lead-fill ring 36 K, the DRP table 26 K — to **name** it. **Why it exists**: the owner's 65 shareware 1p captures of 2026-08-28 show `lg` decaying **224 → 220 → 189 → 159 → 70 KB** across one session and **never recovering**, while `ca` holds at 347–560 KB — plenty reclaimable, none of it assemblable — and the **marginal re-fault rate tracks it exactly**: ~32 % while `lg` = 224, **70–87 %** once `lg` = 70. It even crossed a **level load getting worse** (159 → 70 with `zf` going *up* 55 → 146), so the fragmenters **survive `P_SetupLevel`**. `lg` said we were fragmented; **nothing said by what**. 🔴 **AND IT PROVES THE EVICTION QUESTION CLOSED**: `Z_LargestAllocatable` counts every `PU_PURGELEVEL` block **as free**, so `lg` already means «the largest run obtainable after purging absolutely everything» — a cache purge cannot raise it **by construction**, and the blocks costing contiguity are **necessarily** `PU_STATIC` / `PU_LEVEL`. That is why the recency lever of 2026-08-28 could never have worked, and why the planned «full-purge discriminator» experiment was dropped before it was written. ⚠ Computed by `Z_LargestAllocatable` in its own single pass, which **row 11 calls first** — so `sp` is this frame's, not last frame's. ⚠ `gain` is always > `lg` whenever any unpurgeable block exists; `gain == lg` means the zone has none, which would be a bug. **SPLIT (any build) or CART (≥4 MB)**: `SKY e<v> !<gardes> <ms0> <ms1> <ms2> <ms3> c<cols>` — added 2026-08-25 to size the 3-quadrant HW-sky plan. The four tenths-ms are the **per-view SOFTWARE sky fill**; `e` is the view carrying the HW NBG0 sky, **whose ms reads 0.0 BY CONSTRUCTION** (the `sat_vdp2_sky` branch draws nothing — that IS the HW/software distinction, and it is how a photo identifies the elected view without trusting `e`). 🔴 **`p<px>` BECAME `c<cols>` ON 2026-08-28, BECAUSE PX WAS THE WRONG UNIT AND THIS LEGEND ALREADY SAID SO.** It read: «a pixel count could NOT have answered this: `R_DrawSkyColumn` does a 128-byte per-column memcpy plus the grain loop, so the px→ms factor swings 2–4× with scene geometry». **That swing IS the columns-per-pixel ratio** — the drawer's bill is FIXED PER COLUMN (a one-pixel sky sliver pays the same 128 bytes as a full-height one) plus a short per-pixel loop. So the varying factor is now *printed* instead of inferred. `c` is still a **COUNT**, which is what Ymir is authoritative for, and that splits the labour so neither half lies: **Ymir sizes the JOB** (how many sky columns does a 4p *outdoor* spot really have) and **the console prices the RATE** (ms per column). `ms/c` is also the only honest **cross-build** reading of the sky-drawer change landing with it, because it normalises the scene out — [[interbuild-perf-noise]] forbids comparing the absolutes. `sat_sky_px_view` still drives the election, so it is consumed, not thrown away; if the drawer change leaves the per-pixel loop as the residual, `p` comes back **then** and not before. **All zero in 1p** (the arrays are written only inside `d_main.c`'s split loop and are never reset, hence the mandatory stale-guard). LAST FRAME, like row 2 — never difference it against rows 0–1. ⚠ `e` is the election for the NEXT frame, so it disagrees only on a transition frame. ⚠ this SPENDS the cart half that §P11 and §6.4 both wanted for `ob`/`st`/`gy`. | **cart vs CD-streaming** |
