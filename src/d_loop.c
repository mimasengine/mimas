...
void TryRunTics (void)
{
    ...
    /* SATURN: if DG_GetTicksMs ever returns a garbage large value,
       entertic jumps far ahead and the escape-hatch check
       (I_GetTime()/ticdup - entertic > 0) becomes permanently false,
       causing an infinite wait because BuildNewTic caps maketic at
       gametic + 2, which can never reach gametic + counts.
       Clamp entertic so oldentertics stays sane. */
    if (realtics > 10)
    {
        realtics  = 10;
        entertic  = oldentertics + 10;
    }
