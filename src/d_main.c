...
void D_Display (void)
{
    ...
    /* SATURN: a rogue slave pixel write can corrupt gamestate to a value outside
       the valid gamestate_t enum range (0=GS_LEVEL .. 3=GS_DEMOSCREEN).  If that
       happens, the else-branch below calls Z_FreeTags(PU_LEVEL) which frees live
       BSP/segs/linedefs while the level is still running, turning the next call to
       R_RenderPlayerView into an infinite BSP traversal loop (freeze at ph=4).
       Reset an implausible value to wipegamestate so no wipe is triggered. */
    if ((unsigned)gamestate > GS_DEMOSCREEN)
        gamestate = wipegamestate;
    if (gamestate != wipegamestate)
    {
        /* SATURN: wipe buffers (wipe_StartScreen + EndScreen + shittyColMajorXform)
           need ~192 KB of zone heap.
           - Leaving a level (gamestate != GS_LEVEL): the previous level's PU_LEVEL
             data is no longer needed by either WI_Drawer or the wipe itself, so free
             it first to make room. P_SetupLevel will call Z_FreeTags again harmlessly.
           - Entering a new level (gamestate == GS_LEVEL): P_SetupLevel just loaded
    ...
    I_UpdateNoBlit ();
#if DISPLAY_DEBUG
    dd_t1 = d_ms();  /* after switch/ST_Drawer + I_UpdateNoBlit */
#endif
    ...
    /* SATURN: if DG_GetTicksMs returned a garbage large value just above,
       wipestart >> nowtime and tics = nowtime - wipestart is always a large
       negative int, locking the inner loop forever.  Clamp so that tics
       turns positive within the very next I_GetTime() call. */
    {
        int now_check = I_GetTime();
        if (wipestart > now_check)
            wipestart = now_check - 1;
    }
...
void D_DoAdvanceDemo (void)
{
    ...
    // SATURN: demo lumps are stripped from the cart WAD, so the attract
    // sequence only alternates between the title and credit screens.
    demosequence = (demosequence == 0) ? 2 : 0;
