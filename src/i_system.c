...
{
    // SATURN: zone heap is the whole 1MB low work RAM.
    extern unsigned char *DG_ZoneBase(int *size);
    byte *zonemem = DG_ZoneBase(size);
...
#if !defined(_WIN32) && !defined(__MACOSX__) && !defined(__DJGPP__) && !defined(__sh__)
...
#elif defined(__sh__)
    {
        // SATURN: show the message on the debug console and halt.
