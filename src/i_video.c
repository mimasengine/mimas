...
void I_InitGraphics (void)
{
    ...
    /* SATURN: render into fast high work RAM (also a legal SCU DMA
       source, unlike the low-RAM zone heap) */
    {
        extern byte *DG_FrameBuffer(void);
        I_VideoBuffer = DG_FrameBuffer();
    }
...
{
	// SATURN: I_VideoBuffer is static, nothing to free
}
...
{
    // SATURN: the platform layer blits I_VideoBuffer straight to VDP2
    // VRAM; the generic staging copy below would only waste memory
    // bandwidth and 64KB of RAM.
    DG_DrawFrame();
}

#if 0 /* SATURN */
void I_FinishUpdate_disabled (void)
{
    int y;
